...
2. Enable fast startup and initial composition.
   ...
   This defeats the framework’s optimization of lazily loading assemblies and causes a noticable and unacceptable slowdown in program initialization, especially when allowing all assemblies in an application to contribute parts.

MEF’s catalogs allow a separation between a part instance and its definition.
...
The idea is that you can detect that a particular assembly has changed without fully loading it—perhaps using filesystem-provided metadata such as mtime.
